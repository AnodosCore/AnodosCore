...
- [Building Anodos Core](#building-anodos-core)
...
git clone https://github.com/anodos/anodos
```
...
in the Anodos Core repository under 'Fetch and build inputs' to install sources which require
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/anodos/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/anodos/anodos
...
For example:
```bash
URL=https://github.com/crowning-/anodos.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit anodos=${COMMIT} --url anodos=${URL} ../anodos/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit anodos=${COMMIT} --url anodos=${URL} ../anodos/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit anodos=${COMMIT} --url anodos=${URL} ../anodos/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the anodos git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../anodos/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/anodos.git
SIGPATH=/some/root/path/anodos-detached-sigs.git

./bin/gbuild --url anodos=${BTCPATH},signature=${SIGPATH} ../anodos/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/anodos-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/anodos-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/anodos-osx-build.assert
```
...
[anodos/gitian.sigs](https://github.com/anodos/gitian.sigs/) repository, or if that's not possible to create a pull
request.
