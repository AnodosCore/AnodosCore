 ...
 *   mileanodos-status research, hiring, ongoing, complete, failure, overdue, error //programmatic workflow?
 *   mileanodos-status-error mia, option2, option3
 ...
 *
 *   // network
 *   CAnodosNetwork lvl, network-type, network-status, network-error, mileanodos-status*
 ...
 *   CProjectMileanodos lvl, mileanodos-type, status, status-error, mileanodos-status, mileanodos-status-error, ov
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.anodos.info/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CAnodosNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: AnodosNetwork
// //   -- signature requirements : Key1(User)
// class CAnodosNetworkVariable : public CGovernanceObject
// {
// private:
...
// class CProjectMileanodos : public CGovernanceObject
// {
// private:
