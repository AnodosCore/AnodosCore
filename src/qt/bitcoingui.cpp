    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Anodos Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and anodos: URIs)"));
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Anodos Core"), this);
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Anodos Core"));
    ...
    openAction->setStatusTip(tr("Open a anodos: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Anodos Core help message to get a list with possible Anodos Core command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Anodos Core client") + " " + networkStyle->getTitleAddText();
...
QString updateUrl = "http://pool.anodos.rocks/updater/";
            ...
            if(ThreadSafeMessageBox2(this,"Version '" + ver + "' is available\nDo you want to update now?         ", "Update available",CClientUIInterface::MODAL | CClientUIInterface::ICON_INFORMATION | CClientUIInterface::BTN_YES | CClientUIInterface::BTN_NO ))
            {
                ...
                if (BOOST_OS_WINDOWS) {
                    if (BOOST_ARCH_X86_64) {
                        this->target =  updateUrl + QString("windows/64/anodos-qt.exe");
                    } else if (BOOST_ARCH_X86 || BOOST_ARCH_X86_32) {
                        this->target = updateUrl + QString("windows/32/anodos-qt.exe");
                    }
                } else if (BOOST_OS_LINUX) {
                    if (BOOST_ARCH_ARM) // Pi2
                    {
                        this->target = updateUrl + QString("pi/2/anodos-qt");
                    } else {
                        if (BOOST_ARCH_X86_64) {
                            this->target = updateUrl + QString("linux/64/anodos-qt");
                        } else if (BOOST_ARCH_X86 || BOOST_ARCH_X86_32) {
                            this->target = updateUrl + QString("linux/32/anodos-qt");
                        }
                    }
